...
import { Plus, Search, ChevronLeft, ChevronRight, FolderClosed } from 'lucide-react';
...
interface GroupListClientProps {
  initialData: PaginatedResponse<Group>;
...
export function GroupListClient({
  ...
  const { data: groups, total, totalPages } = initialData;
  const from = total === 0 ? 0 : (page - 1) * limit + 1;
  const to = Math.min(page * limit, total);

  /** Renders the groups table rows — shared between grouped and flat views */
  const renderGroupsTable = (items: Group[]) => (
    <Table>
      <TableHeader>
        <TableRow>
        ...
        {items.length === 0 ? (
          ...
          items.map((group) => (
            ...
            >
              ...
              <TableCell>
                <Badge variant={STATUS_VARIANT_MAP[group.status] || 'secondary'}>
                  {group.status}
                </Badge>
              </TableCell>
              <TableCell className="text-muted-foreground">
    ...
    <div className="space-y-6">
      ...
      {isSuperAdmin ? (
        <TenantGroupedList
          items={groups}
          tenantMap={tenantMap}
          getItemTenantId={(group) => group.tenantId}
          renderTable={renderGroupsTable}
          renderEmpty={() => (
