...
export default function CreatePermissionPage() {
  ...
  async function onSubmit(values: CreatePermissionValues) {
    setSubmitting(true);
    try {
      await clientFetch('/api/proxy/admin/permissions', {
        method: 'POST',
        body: JSON.stringify(values),
      });
      toast.success(t('createPermission'));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-semibold">{t('createPermission')}</h1>
      </div>

      {/* Form */}
      <Card className="max-w-2xl shadow-sm rounded-lg">
        <CardHeader>
          <CardTitle className="text-lg">{t('createPermission')}</CardTitle>
        </CardHeader>
        <CardContent>
          <Form {...form}>
